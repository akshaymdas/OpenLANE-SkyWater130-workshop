...
* Each tool used in the process flow has a configuration tcl file under PDK/sky130A/libs.tech/<tool_name>/*.tcl
...
=== Standard cell design
Standard cell design flow consists of 3 phases
...
`*Outputs*`::  CDL (Circuit Description Language), GDSII, LEF(Library Exchange Format), extracted Spice netlist (.cir), timing, noise, power libs.

=== Standard cell characterization

Standard cell library has cells with different drive strength and functionalities.These cells are characterized by using GUNA tool. 

* The standard cell characterization flow involves
* Link model file of CMOS containing property definitions
* Specify process corners for the cell to be characterized
